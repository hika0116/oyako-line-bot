...
1. Renderの対象サービスで `CHANNEL_SECRET` を先にSecret環境変数として設定する。
2. `CHANNEL_ACCESS_TOKEN`、`OPENAI_API_KEY`、`SUPABASE_URL`、`SUPABASE_KEY` が開発用ではなく本番用であることを確認する。
3. `APP_ENV=production`、`OPENAI_MODEL=gpt-4.1-mini` を確認する。
4. 全単体テストとオフライン12件を実行する。
5. 開発用認証情報で下記の実環境テストを完了する。
6. PRの差分がベースコミット以降の既存機能を削除していないことを再確認する。
...
3. `今日のご飯を考えて` を送り、表示された候補番号を選ぶ。
4. 選んだ料理の詳細レシピまで返信されることを確認する。
5. `夫と意見が合わない` を送り、一般相談として返答されることを確認する。
6. 新しい候補がない状態で `1` を送り、料理選択に進まず質問が一つだけ返ることを確認する。
7. 署名を改変したHTTPリクエストが400になることを確認する。
...
Renderで直前の正常デプロイを再デプロイする場合も、リポジトリ側のリバートを同時に行い、次回デプロイで再発しないようにする。今回DBスキーマ変更はないためDBロールバックは不要。環境変数は `CHANNEL_SECRET` を削除せず維持する。
