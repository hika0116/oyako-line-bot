...
- `recipe_proposal_history`: `user_id` はLINEユーザーIDのため、RLSを有効にしたサービスロール専用テーブルです。anon/authenticated向けポリシーは作りません。将来Supabase Authを導入する場合は、LINEユーザーIDとは別の `auth_user_id` 等を追加し、その列に対して本人用ポリシーを設計します。
